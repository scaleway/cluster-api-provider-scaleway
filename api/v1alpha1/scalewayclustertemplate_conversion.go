...
func (src *ScalewayClusterTemplate) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*infrastructurev1alpha2.ScalewayClusterTemplate)
	log.Printf("ConvertTo: Converting ScalewayClusterTemplate from Spoke version v1alpha1 to Hub version v1alpha2;"+
...
func (dst *ScalewayClusterTemplate) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*infrastructurev1alpha2.ScalewayClusterTemplate)
	log.Printf("ConvertFrom: Converting ScalewayClusterTemplate from Hub version v1alpha2 to Spoke version v1alpha1;"+
		"source: %s/%s, target: %s/%s", src.Namespace, src.Name, dst.Namespace, dst.Name)
