...
func (src *ScalewayMachine) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*infrastructurev1alpha2.ScalewayMachine)
	log.Printf("ConvertTo: Converting ScalewayMachine from Spoke version v1alpha1 to Hub version v1alpha2;"+
...
func (dst *ScalewayMachine) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*infrastructurev1alpha2.ScalewayMachine)
	log.Printf("ConvertFrom: Converting ScalewayMachine from Hub version v1alpha2 to Spoke version v1alpha1;"+
		"source: %s/%s, target: %s/%s", src.Namespace, src.Name, dst.Namespace, dst.Name)
