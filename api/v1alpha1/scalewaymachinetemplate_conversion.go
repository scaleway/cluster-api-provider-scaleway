...
func (src *ScalewayMachineTemplate) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*infrastructurev1alpha2.ScalewayMachineTemplate)
	log.Printf("ConvertTo: Converting ScalewayMachineTemplate from Spoke version v1alpha1 to Hub version v1alpha2;"+
...
func (dst *ScalewayMachineTemplate) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*infrastructurev1alpha2.ScalewayMachineTemplate)
	log.Printf("ConvertFrom: Converting ScalewayMachineTemplate from Hub version v1alpha2 to Spoke version v1alpha1;"+
		"source: %s/%s, target: %s/%s", src.Namespace, src.Name, dst.Namespace, dst.Name)
