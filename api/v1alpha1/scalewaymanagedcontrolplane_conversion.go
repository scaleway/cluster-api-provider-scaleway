...
func (src *ScalewayManagedControlPlane) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*infrastructurev1alpha2.ScalewayManagedControlPlane)
	log.Printf("ConvertTo: Converting ScalewayManagedControlPlane from Spoke version v1alpha1 to Hub version v1alpha2;"+
...
func (dst *ScalewayManagedControlPlane) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*infrastructurev1alpha2.ScalewayManagedControlPlane)
	log.Printf("ConvertFrom: Converting ScalewayManagedControlPlane from Hub version v1alpha2 to Spoke version v1alpha1;"+
		"source: %s/%s, target: %s/%s", src.Namespace, src.Name, dst.Namespace, dst.Name)
