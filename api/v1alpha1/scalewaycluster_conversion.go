...
func (src *ScalewayCluster) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*infrastructurev1alpha2.ScalewayCluster)
	log.Printf("ConvertTo: Converting ScalewayCluster from Spoke version v1alpha1 to Hub version v1alpha2;"+
...
func (dst *ScalewayCluster) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*infrastructurev1alpha2.ScalewayCluster)
	log.Printf("ConvertFrom: Converting ScalewayCluster from Hub version v1alpha2 to Spoke version v1alpha1;"+
		"source: %s/%s, target: %s/%s", src.Namespace, src.Name, dst.Namespace, dst.Name)
