...
func (src *ScalewayManagedCluster) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*infrastructurev1alpha2.ScalewayManagedCluster)
	log.Printf("ConvertTo: Converting ScalewayManagedCluster from Spoke version v1alpha1 to Hub version v1alpha2;"+
...
func (dst *ScalewayManagedCluster) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*infrastructurev1alpha2.ScalewayManagedCluster)
	log.Printf("ConvertFrom: Converting ScalewayManagedCluster from Hub version v1alpha2 to Spoke version v1alpha1;"+
		"source: %s/%s, target: %s/%s", src.Namespace, src.Name, dst.Namespace, dst.Name)
