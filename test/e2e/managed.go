...
// Important! this method assumes the cluster uses a ScalewayManagedControlPlane and MachinePools.
func ApplyManagedClusterTemplateAndWait(ctx context.Context, input ApplyManagedClusterTemplateAndWaitInput, result *ApplyManagedClusterTemplateAndWaitResult) {
	...
	By(fmt.Sprintf("Creating the Scaleway managed workload cluster with name %q using the %q template (Kubernetes %s)", input.ConfigCluster.ClusterName, input.ConfigCluster.Flavor, input.ConfigCluster.KubernetesVersion))
...
func DiscoveryAndWaitForManagedControlPlaneInitialized(ctx context.Context, input DiscoveryAndWaitForManagedControlPlaneInitializedInput, intervals ...interface{}) *infrav1.ScalewayManagedControlPlane {
	...
	By("Getting ScalewayManagedControlPlane control plane")

	var controlPlane *infrav1.ScalewayManagedControlPlane
...
func GetManagedControlPlaneByCluster(ctx context.Context, input GetManagedControlPlaneByClusterInput) *infrav1.ScalewayManagedControlPlane {
	...
	controlPlaneList := &infrav1.ScalewayManagedControlPlaneList{}
	Eventually(func() error {
		return input.Lister.List(ctx, controlPlaneList, opts...)
	}, retryableOperationTimeout, retryableOperationInterval).Should(Succeed(), "Failed to list ScalewayManagedControlPlane object for Cluster %s", klog.KRef(input.Namespace, input.ClusterName))
	Expect(len(controlPlaneList.Items)).ToNot(BeNumerically(">", 1), "Cluster %s should not have more than 1 ScalewayManagedControlPlane object", klog.KRef(input.Namespace, input.ClusterName))
