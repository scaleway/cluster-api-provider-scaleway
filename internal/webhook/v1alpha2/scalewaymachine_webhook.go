...
// ScalewayMachineValidator struct is responsible for validating the ScalewayMachine resource
...
// ValidateCreate implements admission.Validator so a webhook will be registered for the type ScalewayMachine.
func (v *ScalewayMachineValidator) ValidateCreate(_ context.Context, obj *infrav1.ScalewayMachine) (admission.Warnings, error) {
	scalewaymachinelog.Info("Validation for ScalewayMachine upon creation", "name", obj.GetName())
	return nil, nil
}

// ValidateUpdate implements admission.Validator so a webhook will be registered for the type ScalewayMachine.
func (v *ScalewayMachineValidator) ValidateUpdate(_ context.Context, oldObj *infrav1.ScalewayMachine, newObj *infrav1.ScalewayMachine) (admission.Warnings, error) {
	scalewaymachinelog.Info("Validation for ScalewayMachine upon update", "name", oldObj.GetName())

	var allErrs field.ErrorList

	opts := []cmp.Option{ignoreProviderID(oldObj.Spec)}
...
// ValidateDelete implements admission.Validator so a webhook will be registered for the type ScalewayMachine.
func (v *ScalewayMachineValidator) ValidateDelete(_ context.Context, obj *infrav1.ScalewayMachine) (admission.Warnings, error) {
