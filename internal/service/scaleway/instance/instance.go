...
func (s *Service) ensureServer(ctx context.Context) (*instance.Server, error) {
	...
	for _, vol := range s.ScalewayMachine.Spec.AdditionalVolumes {
		if vol.Type != nil && *vol.Type == scratchVolumeType {
			scratchVolumeSizes = append(scratchVolumeSizes, scw.Size(ptr.Deref(vol.Size, 0))*scw.GB)
		}
	}
...
func (s *Service) ensureAdditionalVolumes(ctx context.Context, server *instance.Server) error {
	...
	for i, vol := range s.ScalewayMachine.Spec.AdditionalVolumes {
		volType := ptr.Deref(vol.Type, "block")

		if volType == scratchVolumeType {
		...
		volSize := scw.Size(ptr.Deref(vol.Size, 1)) * scw.GB
		...
		case "block":
			...
			if _, ok := blockVolumesByName[volName]; !ok {
				volume, err := s.ScalewayClient.CreateVolume(ctx, server.Zone, volName, volSize, ptr.Deref(vol.IOPS, 0), s.ResourceTags())
				if err != nil {
